...
class DQNSolver:
    ...
    def __init__(self, observation_space, action_space):
        ...
        self.model = load_model('model_3_3000_sims_5_layers_512_middle_layer_cross_entropy.h5')
...
#            return random.randrange(self.action_space)
        ...
        max_state_value = max(state[0])
        min_state_value = min(state[0])
        for i in range(len(state[0])):
            state[0][i] = (state[0][i] - min_state_value)/(max_state_value - min_state_value)
